...
def main():
	...
	if normalize==True:
		...
		X = np.log1p(X)
		std = np.std(X,axis=0)>=0.05
		mean = np.mean(X,axis=0)>=0.05
	...
	if trials=='auto':
		...
		g = SMD(X,int(kmax/2),trials = trials, n_sub = n_sub, cluster_prior_min = k_prior_min, class_algo='maxmargin')
