"""NOT runnable as shipped. SMD feature selection ran on a SLURM cluster node under Python 3.10
with ray, which `env/environment.yml` deliberately does not install. Its outputs are committed,
which is why everything downstream runs without it. See `smd/README.md`.
"""
...
def TSMD(X,k_guess, trials = None,n_sub = None,prop_algo = 'agglo', class_algo = 'entropy', cluster_prior_min=None, z_score = True, Z_velocity = 0.8):
	N = X.shape[0]
	D = X.shape[1]
	...
	if trials == 'auto':
		...
		while True:
			...
			if z_score:
				...
				Z_d = np.array((gd-g_shuffled.mean())/g_shuffled.std())

				Z_thresh = Z_d-z_temp
				if np.max(Z_thresh) < Z_velocity:
					break
