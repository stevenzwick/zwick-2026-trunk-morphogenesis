...
def _scene_bead_xys_from_row(sr: pd.Series) -> list[list[float]]:
    """Return all bead centers available for one scene row."""
...
def plot_all_scene_outline_before_analysis(
    ...
    for ax, scene_index in zip(axes, scene_ids):
        ...
        bead_xys = _scene_bead_xys_from_row(sr)
        if len(bead_xys):
...
def plot_all_scenes_geometry_before_analysis(
    ...
    force_single_axis_scenes: set = None,
    force_two_axis_label_map: dict = None,
) -> None:
    """
    ...
    """
    scene_ids = bead_df["scene_index"].astype(int).tolist()
    force_single_axis_scenes = set(force_single_axis_scenes or [])
    force_two_axis_label_map = dict(force_two_axis_label_map or {})
    n = len(scene_ids)
    nrows = int(np.ceil(n / ncols))
    ...
    for ax, scene_index in zip(axes, scene_ids):
        ...
        for i, r in enumerate(props):
            ...
            force_single_axis = int(scene_index) in force_single_axis_scenes
            forced_two_axis_label = force_two_axis_label_map.get(int(scene_index), None)
            force_two_axis = (
                forced_two_axis_label is not None
                and int(label_id) == int(forced_two_axis_label)
            )
            ...
            if lobes_payload is None and force_two_axis:
                lobes_payload = split_mask_into_two_lobes(
                    mask,
                    min_peak_distance_px=MERGED_AXIS_MIN_PEAK_DISTANCE_PX * 0.7,
                    min_fraction=MERGED_AXIS_MIN_FRACTION * 0.7,
                )
...
def _coerce_xy_pair(raw) -> tuple[float, float] | None:
    if raw is None:
        return None
...
def _resolve_bead_xy_from_record_or_row(rec: dict, row: pd.Series) -> list[float]:
    """Return a [x,y] bead coordinate for debug payloads, with primary-bead fallback."""
    # 1) ROI JSON explicit bead_xy_px (single-bead legacy).
    xy = _coerce_xy_pair(rec.get("bead_xy_px"))
    if xy is not None:
        return [float(xy[0]), float(xy[1])]

    # 2) Primary bead assignment from summary row (multi-bead scenes).
    px = row.get("primary_bead_x_px", np.nan)
    py = row.get("primary_bead_y_px", np.nan)
    if np.isfinite(px) and np.isfinite(py):
        return [float(px), float(py)]

    # 3) ROI JSON bead_well_x/y (legacy single-bead fields).
    px = rec.get("bead_well_x_px", np.nan)
    py = rec.get("bead_well_y_px", np.nan)
    if np.isfinite(px) and np.isfinite(py):
        return [float(px), float(py)]

    # 4) ROI JSON bead_xys_px list (multi-bead).
    raw_list = rec.get("bead_xys_px", None)
    if isinstance(raw_list, list):
        for item in raw_list:
            xy = _coerce_xy_pair(item)
            if xy is not None:
                return [float(xy[0]), float(xy[1])]

    raise RuntimeError(
        "No valid bead coordinates found for debug payload. "
        "Expected bead_xy_px, primary_bead_x/y, bead_well_x/y, or bead_xys_px."
    )
...
def build_debug_payload(cyst_id: str, summary_source_df: pd.DataFrame | None = None) -> dict:
    ...
    rec["bead_xy_px"] = _resolve_bead_xy_from_record_or_row(rec, row)
...
def plot_scene_qc(scene_index: int) -> None:
    ...
    title = (
        f"Scene {int(scene_index):02d} | annotation_status={sr.get('annotation_status', 'unknown')} | "
        f"n_cysts_detected={int(sr['n_cysts_detected'])} | n_beads={len(bead_xys)}"
    )
...
def _manual_roi_summary_df() -> pd.DataFrame:
    ...
    if _manual_roi_summary_cache is None:
        ...
        candidates = []
        if ROI_SUMMARY_TSV is not None:
            candidates.append(Path(str(ROI_SUMMARY_TSV)))
        candidates.extend(
            [
                ROOT / "results/tables/manual_roi_summary_39locations.tsv",
                ROOT / "results/tables/manual_roi_summary_36locations.tsv",
            ]
        )
        roi_summary_tsv = None
        for cand in candidates:
            try:
                path = resolve_path(str(cand), ROOT)
            except Exception:
                continue
            if path.exists():
                roi_summary_tsv = path
                break
        if roi_summary_tsv is None:
            raise FileNotFoundError(
                "Could not locate a manual ROI summary TSV. Tried: "
